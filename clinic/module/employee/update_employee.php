<?
	if($_POST[u_idcard]=="" || $_POST[u_username]=="" || $_POST[u_password]==""){//ตรวจสอบค่าว่างทั้งหมด
		$_SESSION[warning]="กรุณากรอกข้อมูลพนักงานให้ครบด้วยค่ะ.";
		echo "<script type='text/javascript'>window.location='index_panel.php?module=employee&file=edit_employee&uid=$_POST[uid]';</script>";
	}else{
		...
		if(mysql_query($sql) or die (mysql_error())){
			$_SESSION[success]="แก้ไขข้อมูลพนักงานสำเร็จแล้ว";
			echo "<script type='text/javascript'>window.location='index_panel.php?module=employee&file=show_employee&uid=$_POST[uid]';</script>";
		}
		else{
			$_SESSION[error]="ไม่สามารถแก้ข้อมูลพนักงานได้";
			echo "<script type='text/javascript'>window.location='index_panel.php?module=employee&file=edit_employee';</script>";
		}
