            <div class="row">
                <div class="col-lg-12 hidden-print">
                    <h1 class="page-header">รายงานรายรับ</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                	<div class="row hidden-print">
                    	<div class="panel-body">
                            <form action="index_panel.php?module=report&file=report_revenue" method="post">
								...
								<div class="form-inline" style="float:left; margin-right:5px;">
                                	<div class="form-group input-group">
                                    	<?	if($_POST[a_status]=="0"){ $sel1="selected";}else{$sel0="";}
												if($_POST[a_status]=="1"){ $sel1="selected";}else{$sel1="";}
												if($_POST[a_status]=="2"){ $sel2="selected";}else{$sel2="";}
												if($_POST[a_status]=="3"){ $sel3="selected";}else{$sel3="";}
										?>
										<span class="input-group-addon">สถานะ</span>
											<select class="form-control" name="a_status" >
												<option value="0" <? echo $sel0; ?>>ทั้งหมด</option>
                                                <option value="1" <? echo $sel1; ?>>ค่ารักษา</option>
												<option value="2" <? echo $sel2; ?>>ขายสินค้า</option>
                                                <option value="3" <? echo $sel3; ?>>อื่นๆ</option>
											</select>
									</div>
                                </div>
                                	<? $a=$_GET[date_start]; ?>
                    ...
                    <div class="row hidden-print">
                        <div class="panel-body">
							...
							if($_POST[a_status]=="0"){
								$result_revenue=mysql_query("SELECT Reve_ID, User_ID, Pat_ID, Reve_Name, Reve_Price, Reve_Type, Reve_Date FROM revenue WHERE Reve_Date Between '$_POST[date_start]' and '$_POST[date_end]' ORDER BY Reve_ID ASC");
							}else{
								$result_revenue=mysql_query("SELECT Reve_ID, User_ID, Pat_ID, Reve_Name, Reve_Price, Reve_Type, Reve_Date FROM revenue WHERE Reve_Date Between '$_POST[date_start]' and '$_POST[date_end]' AND Reve_Type='$_POST[a_status]' ORDER BY Reve_ID ASC");
							}
